...
try {
	...
	List<UIVersionBean> result = GenericUtil.transformList(assetIds, new GenericUtil.Transformer<AssetId, UIVersionBean>() {
		public UIVersionBean transform(AssetId assetId) {
			...
			try {
				AssetData assetData = assetService.read(assetId, Arrays.asList(IAsset.NAME, IAsset.DESCRIPTION));
				...
				PermissionBean<VersioningAuthorizationBean> undoCheckoutPermission = authService.canUndoCheckout(assetId);
				if(undoCheckoutPermission.isPermitted()) {
					VersioningMessageCollector vc = new VersioningMessageCollector();
					VersionBean revision = versioningService.undoCheckout(assetId, vc);
					if(revision == null) {
					...
					VersioningAuthorizationBean authBean = undoCheckoutPermission.getEntity();
					...
					uiBean.setSuccess(false);
					uiBean.setDetail(undoCheckoutPermission.getMessages().get(0));
				}

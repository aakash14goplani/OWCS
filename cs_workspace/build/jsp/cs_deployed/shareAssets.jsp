...
%><cs:ftcs>
	...
	try{
		...
		String source_pubid = "1492586991364"/*source_website*/, destination_pubid = "1374098698899";/*target_website*/
		if( Utilities.goodString(assetType) ){
			if(assetIdList != null){
				...
				query = (Utilities.goodString(assetIdList)) ? "select asset.id, asset.name from " + assetType + " asset, AssetPublication ap where asset.status != 'VO' and asset.id in(" + assetIdList + ") and ap.assetid in(" + assetIdList + ") and ap.pubid = " + source_pubid : "select asset.id, asset.name from " + assetType + " asset, AssetPublication ap where asset.status != 'VO' and asset.id = ap.assetid and ap.pubid = " + source_pubid;
						...
						%><ics:listloop listname="pubid_list">
					    	...
					    	<ics:if condition='<%="true".equalsIgnoreCase(run) %>'>
					    		<ics:then>
	            					...
	            					<asset:share name="publishAsset" publist='<%=destination_pubid %>'/>
					        	</ics:then>
					        </ics:if>
					     </ics:listloop>		     
					</ics:then>
				...
				%><ics:logmsg msg='<%="Before : "  + ics.GetErrno()%>'/>

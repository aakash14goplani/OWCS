...
<%@page import="java.util.ArrayList"%><cs:ftcs><%-- /TemplateSave
...
<ics:if condition='<%=ics.GetVar("tid")!=null%>'><ics:then><render:logdep cid='<%=ics.GetVar("tid")%>' c="Template"/></ics:then></ics:if>
	...
	if( ics.GetErrno() == 0 ){
		...
		if( isMember ) {
	%>
			<asset:create name="currentAsset" type="Template"/>
            <%-- Don't give empty elements to over write existing element if using existing--%>
            <% if (!request.getParameter("useExistingElementCatalog").equals("true"))
                {
            %>
                <ics:setvar name="element:Total" value="1"/>
            <%
                }
                else
                {
            %>
                <ics:setvar name="element:Total" value="0"/>
            <%
                }
            %>
            <ics:setvar name="element:0:siteentry:Total" value="1"/>
			...
			<asset:set name="currentAsset" field="subtype" value='<%=request.getParameter("subtype")%>'/>
			<asset:set name="currentAsset" field="description" value='<%=request.getParameter("description")%>'/>
			<asset:set name="currentAsset" field="element" value='<%=request.getParameter("element")%>'/>
			<asset:set name="currentAsset" field="applicablesubtypes" value='<%=request.getParameter("applicablesubtypes")%>'/>
			<asset:set name="currentAsset" field="ttype" value='<%=request.getParameter("ttype")%>'/>
			<asset:set name="currentAsset" field="Called" value='<%=request.getParameter("Called")%>'/>
			<ics:setvar name="element:0:elementname" value='<%=request.getParameter("elementname")%>'/>
			<ics:setvar name="element:0:elementdescription" value='<%=request.getParameter("elementdescription")%>'/>
			...
			<ics:setvar name="element:0:cscacheinfo" value='<%=request.getParameter("cscacheinfo")%>'/>
			<ics:setvar name="element:0:sscacheinfo" value='<%=request.getParameter("sscacheinfo")%>'/>
			<ics:setvar name="element:0:csstatus" value="live"/>
			<ics:setvar name="element:0:siteentry:0:pagename" value='<%=request.getParameter("pagename1")%>'/>
			<%
			String prefixStr = "element:0:siteentry:0:defaultarguments:";
			...
			%>
				<ics:setvar name="element:0:siteentry:0:defaultarguments:Total" value='<%=totalDefaultArgs%>'/>
			<%
			...
			<assetid>Template:<ics:getvar name="assetid"/></assetid>
			Success
			<%
					}
				} else {

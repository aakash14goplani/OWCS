...
<script type="text/javascript">
	...
	var translateAssetTab = function (dimId, dimType, dimName, dimNameDesc, assetId, assetType) {
		var docId = parent.fw.util.buildDocId({type: assetType, id: assetId});
		var thisView = SitesApp.getActiveView();
		SitesApp.event(docId, "translate", {id: dimId, type: dimType, name: dimName, nameDesc: dimNameDesc});
...
for ( IList row : new IterableIListWrapper( ics.GetList("listLocales") ) )
{
    ...
    else if (!bRevisionInspect)
    {
		...
		<% if("ucform".equals(ics.GetVar("cs_environment"))){%>
			<ics:setvar name="referURL" value='<%="javascript:translateAssetTab(\'" + row.getValue("id") + "\',\'Dimension\',\'" + org.apache.commons.lang.StringEscapeUtils.escapeJavaScript(sLocale)
									+ "\',\'" + org.apache.commons.lang.StringEscapeUtils.escapeJavaScript(sLocaleDesc)  + "\',\'" + ics.GetVar("id")  + "\',\'"  + ics.GetVar("AssetType") + "\');"%>' />
		<% } else { %>

...
<%
	//
// OpenMarket/Gator/FlexibleAssets/Common/AssocAttr
//
// INPUT
//
// OUTPUT
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function delete_textfield(form, atrributname, attrib)
	{
		var attribcount = attrib + "VC";
		var obj = document.forms[0].elements[0];
		
		if (obj && obj.form.elements['doSubmit'].value == "yes")
		{
			var totalattirbcount = obj.form.elements[attribcount].value;
			if (totalattirbcount > 1)
			{	
				var j=0;
				var upos = atrributname.lastIndexOf( "_" );
				var num = atrributname.substring( upos+1 );
				
				for(var i=1; i<=totalattirbcount; i++)
				{
					if (num != i )
					{
						j=j+1;
						obj.form.elements["Attribute_"+attrib+"_"+j].value = obj.form.elements["Attribute_"+attrib+"_"+i].value ;
					}
				}
				obj.form.elements[attribcount].value = j;
			}   
...
<string:encode varname="flextemplateidenc" variable="flextemplateid"/>
<INPUT TYPE="hidden" NAME="flextemplateid" VALUE="<%=ics.GetVar("flextemplateidenc")%>" />
...
<%
	...
	if (ics.GetVar("tagtype").equals("flexgrouptemplates"))
	{
		tmplID = ics.GetVar("flexgrouptemplateid");
	}
	else
	{
		tmplID = ics.GetVar("flextemplateid");
	}
...
<%
		...
		for (int i = 1; tmplattrlist.moveTo(i); i++)
		{
			...
			if (enabledAttributes == null || enabledAttributes.containsKey(tmplattrlist.getValue("name")))
			{
				...
				ics.SetVar("attrassettdef", tmplattrlist.getValue("assettypedef"));
	...
	%><string:encode varname="currentAttrNameorDesc" variable="currentAttrNameorDesc"/><%			
...
<%
				...
				boolean isSingleStyle = ics.GetVar("EditingStyle").equals("single");
				String editingStyleStatus = isSingleStyle ? "single" :  "multiple";
				// this is a convenience for single valued attrs, multi valued attrs need to call this again in the value loop
%>
<input TYPE="hidden" NAME='<%=ics.GetVar("AttrName")+"_type"%>' VALUE='<%=editingStyleStatus%>'/>
...
<%
				if (!(ics.GetVar("AttrName").equals("GAProductSet")))
				{
					...
					{
						//out.println("OUR CODE EXECUTED <BR/>");
...
<%
							...
							attrVCValue = 0 == attrValueList.numRows() ? 1 : attrValueList.numRows();
...
<%
					...
					<SCRIPT>
						(function(){
							var createdNode,
								vcNodeName = "<%= ics.GetVar("AttrName") %>VC",
								vcNodeValue = "<%= attrVCValue %>",
								vcNodes = dojo.query("input[name=" + vcNodeName + "]", document.forms[0]);
							if(vcNodes.length === 0)
								createdNode = dojo.create("input", {
									"name": vcNodeName,
									"type": "hidden",
									"value": vcNodeValue
								}, document.forms[0], "last");	
						}());
					</SCRIPT>
					<%	
					// doDefaultDisplay
...
<SCRIPT>
	(function(){
		var createdNode,
			reqNodeName = "A<%=numAttrs%>_RequireInfo",
			reqNodeValue = "<%= ics.GetVar("RequireInfo") %>",
			requireInfoNodes = dojo.query("input[name=" + reqNodeName + "]", document.forms[0]);
		if(requireInfoNodes.length === 0)
			createdNode = dojo.create("input", {
				"name": reqNodeName,
				"type": "hidden",
				"value": reqNodeValue
...
<%
			...
			{
%>
<STRING:ENCODE VARIABLE="ccsourceid" VARNAME="ccsourceidenc"/>
<input TYPE="hidden" NAME="ccsourceid" VALUE="<%=ics.GetVar("ccsourceidenc")%>" />
<%
			}
		}

...
<%//
// OpenMarket/Gator/FlexibleAssets/Common/AssetGather
//
// INPUT
...
private static final String FLEXGROUP = "flexgroup";
private static final String FLEXASSET_DEFCOL = "flextemplateid";
private static final String FLEXGROUP_DEFCOL = "flexgrouptemplateid";
private static final String FLEXASSET_FIELDLIST = "name,description,Webreference,fwtags,flextemplateid,externalid,template,filename,path,renderid,Groups,Relationships,Dimension,Dimension-parent,startdate,enddate,SegRating";
private static final String FLEXGROUP_FIELDLIST = "name,description,Webreference,fwtags,flexgrouptemplateid,template,filename,path,Groups,Relationships,Dimension,Dimension-parent,startdate,enddate,SegRating";
%>
...
String sFlexType = null;
String sFlexDefCol = null;
...
if (bIsFlexAsset) {
    sFlexType = FLEXASSET;
    sFlexDefCol = FLEXASSET_DEFCOL;
    sFieldList = FLEXASSET_FIELDLIST;
} else {
    sFlexType = FLEXGROUP;
    sFlexDefCol = FLEXGROUP_DEFCOL;
    sFieldList = FLEXGROUP_FIELDLIST;
}
if (bIsFlexAsset)
{
...
%>
	<ics:callelement  element='OpenMarket/Gator/FlexibleAssets/Common/RatingRuleSet'/>
	<!--  Advanced Collection  stuff -->
	<ics:callelement element='OpenMarket/Gator/FlexibleAssets/Common/AGRecommendations'/>
	<!--  Advcoll ends here-->
...
%>
	<!-- 
	PR26282 - Logically, AssetChildFormNewGather should be called before asset:gather because the
	element sets variables to be gathered later. However prior to calling AssetChildFormNewGather, 
	we need to know the subtype which is known only after the asset:gather. Fortunately, the gathering
	of assetchildren/associations is handled elsewhere and not by asset:gather which makes it 
	possible to call AssetChildrenFormNewGather after asset:gather here. 
	-->
...
<%
	...
	ics.runTag(sFlexType+".clearparents", vN);
...
<%
	...
	else
	{
		...
		int numparents = Integer.parseInt(ics.GetVar("numParents"));
		if (numparents != 0) {
			ics.SetCounter("parentCount", 1);
			for (int i = 0; i < numparents; i++) {
				ics.ClearErrno();
				String currentParents = ics.GetVar("_ParentDef_"+ics.GetCounter("parentCount")+"_SelectedParents_");
				String currentParent = null;
				if (currentParents != null && currentParents.length()!=0) {
					java.util.StringTokenizer parentgroups = new java.util.StringTokenizer(currentParents, ";");
					while (parentgroups.hasMoreTokens()) {
						currentParent = parentgroups.nextToken();
						%><hash:add name='refreshHash' value='<%="Self:"+currentParent%>'/><%
						vN = new FTValList();
						vN.setValString("NAME", "theCurrentAsset");
						vN.setValString("ID", currentParent);
						ics.runTag(sFlexType+".addparent", vN);
					}
				}

                   %><!--Puts the asset ids of all the original parents of an asset in refreshHash to refresh that parent node--> <%
				if(null != ics.GetVar("lstOfParents")){   
					String[] OrigParents = ics.GetVar("lstOfParents").split(";");

					for (int j = 0; j < OrigParents.length; j++){

						String OrigParent = OrigParents[j];
						if (OrigParent != null){
						%>
						<hash:add name='refreshHash' value='<%="Self:"+OrigParent%>'/>
						 <%
						}
					}
				}   

                ics.SetCounter("parentCount", ics.GetCounter("parentCount")+1);
			}
%>
			<hash:hasdata name='refreshHash' varname='hasData'/>
<%
			if (ics.GetVar("hasData").equals("false"))
			{
%>
				<hash:add name='refreshHash' value='<%="Root:"+ics.GetVar("AssetType")%>'/>
<%
			}
		}
		else
...
%>
		<!-- Make sure that the Parents that are not shared to this site are not
			wiped out for other sites as the UI wipes out the relationship info  -->
		<!--  create a hashed list  of Possible Parents for this Asset  -->
<%
		if (ics.GetVar("cs_AbsenteeParents")!=null && ics.GetVar("cs_AbsenteeParents").length()!=0)
		{
			java.util.StringTokenizer siteparents = new java.util.StringTokenizer(ics.GetVar("cs_AbsenteeParents"), ";");
			while (siteparents.hasMoreTokens())
			{
				vN = new FTValList();
                		vN.setValString("NAME", "theCurrentAsset");
                		vN.setValString("ID", siteparents.nextToken());
                		ics.runTag(sFlexType+".addparent", vN);
			}
		}
	} // not remotepost
...
<%
					...
					if (ics.GetVar("EditingStyle").equals("O"))
					{
						...
						else
						{
							...
							{
								String value = ics.GetVar("cs_"+ics.GetVar("n")+"_ordinal_"+String.valueOf(valueCount));
								if(!Utilities.goodString(value))
								{
						...
						{							
							...
							if ( filename != null )
							{
								...
								if (ics.GetVar("tempid")!=null)
								{
                                    if (numValues > ics.GetCounter("TCounter") - 1 - ics.GetCounter("deleteCount"))
									{
										...
										/*
											When a new file is uploaded any item existing at the same BLOBindex 
											has to be deleted. But this BLOBIndex might have changed, after 
											re-ordering of multi-valued blob. So we need to find the right row (multi-value) entry
											to be deleted based on [ cs_{Attrname}_ordinal_ ] value.
											
											For e.g           1 -> entry 1
															  2 -> entry 2
															  3 -> entry 3 , be the original order after opening the editing page from
															  Advanced UI.
															  
												After re-ordering of first 2 entries
												
																1 -> entry 2 . 
																2 -> entry 1 . Browse a new file here after re-ordering.
																3 -> entry 3 . 
																
											    Here the replacement should happen for entry 1, whose original position was 1 and current position is 2.
											    Below code finds the correct BLOBIndex from {cs_Attrname_ordinal_} field.
										 */
										String startsCompare = "cs_"+ics.GetVar("n")+"_ordinal_";
										for(int k=0;k<nCounterValue;k++)
										{
											String s = startsCompare + (k+1);
											String value = ics.GetVar(s);
											if(Utilities.goodString(value) && (Integer.parseInt(value) == BLOBindex + 1))
											{
												BLOBindex = k;
												break;	
											}	
							...
							else
							{
								...
								{	
									...
									if (isID(ics.GetVar(attribute))) {
										ics.SetVar("tempid", ics.GetVar(attribute));
										ics.SetVar(tmplattrlist.getValue("assetid") + "_isValueId", "true");
									}
									
									if (!"true".equalsIgnoreCase(ics.GetVar("isReposted")) && 
											!"true".equalsIgnoreCase(ics.GetVar(tmplattrlist.getValue("assetid") + "_isValueId"))) {
										vN = new FTValList();
										vN.setValString("ID", ics.GetVar("currentTempID"));
										ics.runTag("tempobjects.delete", vN);
									}									
									...
									if (!"true".equalsIgnoreCase(ics.GetVar(tmplattrlist.getValue("assetid") + "_isValueId"))) {
										vN = new FTValList();
										vN.setValString("FILE", ics.GetVar(attribute+"_tfile") );
										vN.setValString("BINVARNAME", ics.GetVar("cs_CurrentInputName"));
										vN.setValString("VARNAME", "tempid");
										//System.out.println("!!! [AG] - TO.set 2 - tempId: " +ics.GetVar(attribute+"_tfile"));
										ics.runTag("tempobjects.set", vN);
									}
...
<%
							String ids = ics.GetVar("tempids");
							String[] tempidTokens = ids.split(",");
							int i = 0;
							// setting the initial capacity to avoid multiple calls to increase capacity and copying data,
							// in case of large number of multi-valued data
							StringBuilder tempIndexForids = new StringBuilder(ids.length()+2*tempidTokens.length);
							while (i<tempidTokens.length)
							{
								if(newFileUploaded)
								{ 
									// Prepare a string to preserve the order of iteration by keeping track of i.
									
									tempIndexForids.append( tempidTokens[i]).
													append(":").
													append(i+1).
													append(",");
								}
%>
								<listobject:addrow name='temp'>
									<listobject:argument name='id' value='<%=tempidTokens[i]%>'/>
								</listobject:addrow>
<%
									i++;
							}
					...
					if (ics.GetVar("type").equals("url") && "false".equalsIgnoreCase(ics.GetVar("takeDetour")))
					{
						...
						if ( filename != null )
						{
							...
							if (ics.GetVar("tempid")!=null)
							{
								...
								IListBasic attrvals = (IListBasic) ics.GetList("AttrValueList");
%>
								<ics:if condition="<%= attrvals.numRows() > 0 %>">
								<ics:then>
									<ics:callelement element='OpenMarket/Gator/Util/FixBLOBList'>
										<ics:argument name='cs_ListToReplace' value='AttrValueList'/>
									</ics:callelement>
...
%>
		<INPUT TYPE="HIDDEN" NAME="__TEMPOBJECTS__" VALUE="<%=ics.GetVar("__TEMPOBJECTS__")%>"/>
<%		
	}
}
%>

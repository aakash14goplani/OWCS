...
<c:if test="${not empty asset.relatedImage}">
	<a href='<ics:getvar name="articleUrl" />'><render:calltemplate tname="Summary/Large" c="AVIImage" cid="${asset.relatedImage.id}" style="element" /></a>
</c:if>
<div>
	<p class="descr"><insite:edit field="abstract" value='${asset["abstract"]}' params='{width: "165px"}'/></p>
	<ul>
	<c:set var="nbLinks" value='<%=ics.GetVar("nb-links") %>' />
	<c:forEach var="article" items="${asset.relatedStories}" end='${nbLinks - 1}'>

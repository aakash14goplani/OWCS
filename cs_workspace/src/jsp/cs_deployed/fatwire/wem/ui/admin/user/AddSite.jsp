...
<%//
// fatwire/wem/ui/admin/user/AddSite
//
// INPUT
//
// OUTPUT
...
<cs:ftcs>
 ...
 populate = function(obj) {
  ...
  //Preprocessing is only needed if the sites are available
...
%>
			...
			<div class="row">
			<select dojotype="fw.dijit.UITransferBox" id="addSite_TransferBox" name="addSite_TransferBox" required="true" size="10" store="sitesstore" storeStructure="{title:'name',value:'name'}" title1='<%=ics.GetVar("_XLAT_1")%>' title2='<%=ics.GetVar("_XLAT_2")%>'>
			</select>
			</div>
			<div class="clear"></div>
			<div class="buttonsrow">
			<button id="btnContinue" dojoType="fw.ui.dijit.Button" buttonType="Continue wemButtonStyle" buttonStyle="grey" onClick="dijit.byId('addSite_TransferBox').selectAll();AdminUIManager.continueForm('<%="AssignRoles?fw:adminUI:identifier=" + _identifier + "&fw:adminUI:submitPage=user/AssignSites"%>');">
			 <xlat:stream  key='fatwire/wem/admin/common/actions/Continue'/>
			 </button>
			<span class="button" style="top:1px;" onclick="dijit.byId('addSite_TransferBox').selectAll();AdminUIManager.loadPage('<%="user/AssignSites?fw:adminUI:identifier=" + _identifier%>','prev');"><xlat:stream key="fatwire/wem/admin/common/actions/Cancel"/></span>
			</div>
			</div><%--end of UIForm--%>
			</div>

			<ics:callelement element="fatwire/wem/ui/admin/Description">
				<xlat:lookup key="fatwire/wem/admin/user/addSite/DescriptionTitle" varname="_XLAT_"/>
				<ics:argument name="Description:heading" value='<%=ics.GetVar("_XLAT_")%>'/>
				<xlat:lookup key="fatwire/wem/admin/user/addSite/DescriptionBody" varname="_XLAT_" encode="false"/>

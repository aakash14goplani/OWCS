...
%><cs:ftcs>
	...
	<%-- <assetset:getattributevalues name="fetchPageAssetDetails" typename="PageAttribute" attribute="cselement_dropzone" listvarname="csElementDropzoneList"/>
	<assetset:getattributevalues name="fetchPageAssetDetails" typename="PageAttribute" attribute="site_entry_dropzone" listvarname="siteEntryDropzoneList"/> --%>

	<%-- <asset:list type='<%=ics.GetVar("c")%>' list="asset" field1="id" value1='<%=ics.GetVar("cid")%>' />
	<ics:listloop listname="asset">
		<ics:listget listname="asset" fieldname="template" output="template" />
		Template: <ics:getvar name="template"/><br/>
		<render:gettemplateurl outstr="pageURL" tname='<%=ics.GetVar("template")%>' args="c,cid" />
		Template URL: <ics:getvar name="pageURL"/><br/>
		<ics:removevar name="template"/>
		<ics:removevar name="pageURL"/>
	</ics:listloop> --%>
					...
					<ics:if condition='<%= null != ics.GetList("contentEditableDropzoneList") && ics.GetList("contentEditableDropzoneList").hasData()%>'>
						<ics:then>
							...
							<ics:listloop listname="contentEditableDropzoneList">
	    						...
	    						<insite:calltemplate tname='<%=ics.GetVar("templateName") %>' c="Practice_C" cid='<%=ics.GetVar("assetId") %>' variant="PresentationEditing|PresentationEditing_2" slotname='<%=ics.GetVar("assetId") + ":ContentEditableDropzone" %>' />
					...
					<ics:if condition='<%= null != ics.GetList("insiteDropzoneList") && ics.GetList("insiteDropzoneList").hasData()%>'>
						<ics:then>
							...
							<ics:listloop listname="insiteDropzoneList">
								...
								<asset:load name="loadCurrentAsset" type="Practice_C" objectid='<%=ics.GetVar("assetId")%>'/>
	    						<asset:get name="loadCurrentAsset" field="template" output="templateName" />
	    						<insite:calltemplate tname='<%=ics.GetVar("templateName") %>' c="Practice_C" cid='<%=ics.GetVar("assetId") %>' variant="PresentationEditing|PresentationEditing_2" slotname='<%=ics.GetVar("assetId") + ":InsiteDropZone" %>' />
	    						<%-- <render:calltemplate tname='<%=ics.GetVar("templateName") %>' c="Practice_C" cid='<%=ics.GetVar("assetId") %>' /> --%>
								<ics:removevar name="templateName"/>
								<ics:removevar name="assetId"/>
							</ics:listloop>
					...
					<ics:if condition='<%= null != ics.GetList("bodyTextDropzoneList") && ics.GetList("bodyTextDropzoneList").hasData()%>'>
						<ics:then>
							...
							<ics:listloop listname="bodyTextDropzoneList">
								...
								<asset:load name="loadCurrentAsset" type="Practice_C" objectid='<%=ics.GetVar("assetId")%>'/>
	    						<asset:get name="loadCurrentAsset" field="template" output="templateName" />
	    						<insite:calltemplate tname='<%=ics.GetVar("templateName") %>' c="Practice_C" cid='<%=ics.GetVar("assetId") %>' variant="PresentationEditing|PresentationEditing_2" slotname='<%=ics.GetVar("assetId") + ":BodyTextDropzone" %>' />
	    						<%-- <render:calltemplate tname='<%=ics.GetVar("templateName") %>' c="Practice_C" cid='<%=ics.GetVar("assetId") %>' /> --%>
								<ics:removevar name="templateName"/>
								<ics:removevar name="assetId"/>
							</ics:listloop>
					...
					<ics:if condition='<%= null != ics.GetList("priceDateDropzoneList") && ics.GetList("priceDateDropzoneList").hasData()%>'>
						<ics:then>
							...
							<ics:listloop listname="priceDateDropzoneList">
								...
								<asset:load name="loadCurrentAsset" type="Practice_C" objectid='<%=ics.GetVar("assetId")%>'/>
	    						<asset:get name="loadCurrentAsset" field="template" output="templateName" />
	    						<insite:calltemplate tname='<%=ics.GetVar("templateName") %>' c="Practice_C" cid='<%=ics.GetVar("assetId") %>' variant="PresentationEditing|PresentationEditing_2" slotname='<%=ics.GetVar("assetId") + ":PriceDateDropzone" %>' />
	    						<%-- <render:calltemplate tname='<%=ics.GetVar("templateName") %>' c="Practice_C" cid='<%=ics.GetVar("assetId") %>' /> --%>
								<ics:removevar name="templateName"/>
								<ics:removevar name="assetId"/>
							</ics:listloop>
					...
					<ics:if condition='<%= null != ics.GetList("inmageDropzoneList") && ics.GetList("inmageDropzoneList").hasData()%>'>
						<ics:then>
							...
							<ics:listloop listname="inmageDropzoneList">
	    						...
	    						<asset:get name="loadCurrentAsset" field="template" output="templateName" />
	    						<insite:calltemplate tname='<%=ics.GetVar("templateName") %>' c="ImageAsset" cid='<%=ics.GetVar("assetId") %>' variant="PresentationEditing|PresentationEditing_2" slotname='<%=ics.GetVar("assetId") + ":ImageDropzone" %>' />
